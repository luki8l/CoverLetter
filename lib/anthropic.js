...
// ─── Follow-up Email ─────────────────────────────────────────────────────────
export function buildFollowUpPrompt({ type, company, jobTitle, interviewerName, discussedTopics, appliedDate, language }) {
  const lang = language === 'Deutsch'
    ? 'Write the entire email in German. Use natural, modern German — not stiff or overly formal.'
    : 'Write in English.';

  const rules = `
Rules — follow strictly:
- Under 120 words total (subject line not counted).
- No sycophantic openers like "I hope this email finds you well" or "I wanted to reach out."
- No filler phrases. Every sentence must earn its place.
- Specific > generic. Reference real details where provided.
- Professional but human. Should not read like a template.
- Output format: first line is "Subject: [subject line]", then a blank line, then the email body.
- Output ONLY the email. No explanation, no preamble.`;

  const typePrompts = {
    'follow-up': `You are writing a professional application follow-up email, sent roughly one week after applying.
Goal: politely check on status, reaffirm interest, invite a response. Not needy or aggressive.
${appliedDate ? `Application was submitted on: ${appliedDate}.` : ''}`,

    'thank-you': `You are writing a post-interview thank-you email to send within 24 hours of an interview.
This is one of the most underused tools in job hunting — a specific, thoughtful thank-you often tips the decision.
Goal: thank them specifically, reference something concrete from the conversation, reaffirm one reason you're excited about the role.
Interviewer name: ${interviewerName || 'not provided — use "Dear [Name]" as placeholder'}.
${discussedTopics ? `Topics discussed in the interview: ${discussedTopics}` : 'No specific topics provided — write a strong general thank-you that still feels specific.'}`,

    'rejection': `You are writing a graceful response to a job rejection.
Goal: thank them genuinely, keep the door open for future opportunities, leave a positive lasting impression.
Tone: dignified, not desperate. No begging for reconsideration. Short and clean.`,
  };

  return `${typePrompts[type] || typePrompts['follow-up']}
...
// ─── Cover Letter Grader ─────────────────────────────────────────────────────
export function buildGradePrompt({ coverLetter, jobDescription, jobTitle, company }) {
  return `You are a senior recruiter and hiring manager. Grade this cover letter ruthlessly and honestly.
...
--- COVER LETTER TO GRADE ---
${coverLetter}
--- END ---

Score on exactly these 5 dimensions. Be calibrated — most letters score 55-78 total. A 90+ is rare and exceptional.

Return ONLY valid JSON — no markdown, no code fences, nothing else:
{
  "total": <integer 0-100, sum of all dimensions>,
  "hook": <integer 0-20, opening paragraph — is it specific, does it hook immediately, or is it generic filler?>,
  "specificity": <integer 0-25, are there concrete numbers/results/skills, or vague claims?>,
  "atsMatch": <integer 0-25, how well do the keywords align with the job description requirements?>,
  "conciseness": <integer 0-15, right length (200-350 words), no padding, every sentence earns its place?>,
  "persuasion": <integer 0-15, overall: does this make you want to call this person?>,
  "verdict": "<one of: Needs Work | Decent | Good | Strong | Exceptional>",
  "tips": [<2-3 specific, actionable improvement tips — reference actual text from the letter, not generic advice. Only include if total < 90. Empty array if 90+.>]
}

Scoring guide: hook 16-20 = strong, 10-15 = average, 0-9 = weak. Be honest.`;
}

// ─── LinkedIn Outreach ───────────────────────────────────────────────────────
export function buildLinkedInPrompt({ type, yourName, yourBackground, recipientName, recipientTitle, company, role, mutualContext, language }) {
  const lang = language === 'Deutsch'
    ? 'Write entirely in German. Natural, professional German — not stiff or formal.'
    : 'Write in English.';

  const typeGuides = {
    'cold-recruiter': `You are writing a cold LinkedIn message from a job seeker to a recruiter or hiring manager.
Goal: get a response. Short, specific, confident. Not desperate or generic.
This is a direct message (InMail or message after connecting) — up to 300 words is fine, but shorter is better.`,

    'open-role': `You are writing a LinkedIn message to a recruiter or hiring manager about a specific open role.
The sender saw the job posting and wants to express strong interest and get a response.
Goal: stand out from the pile of generic "I am interested in this role" messages.`,

    'connection-request': `You are writing the NOTE that accompanies a LinkedIn connection request.
CRITICAL: Must be UNDER 300 characters total (LinkedIn's hard limit). Count every character.
Make it personal, specific, and non-salesy. No "I'd like to add you to my network."`,
  };

  return `${typeGuides[type] || typeGuides['cold-recruiter']}

${HUMAN_TONE_RULES}

--- DETAILS ---
Sender name: ${yourName || '[Your Name]'}
Sender background: ${yourBackground || '(not provided)'}
Recipient name: ${recipientName || 'not provided — use "Hi [Name]" as placeholder'}
Recipient title/role: ${recipientTitle || 'Recruiter / Hiring Manager'}
Company: ${company}
${role ? `Role they are interested in: ${role}` : ''}
${mutualContext ? `Mutual connection or context: ${mutualContext}` : ''}
${lang}

Rules:
- Open with something specific — reference their company, a recent post, a mutual connection, or the specific role. NOT "I hope you're doing well."
- Be direct about the ask in one sentence.
- No buzzwords. No "I am passionate about" or "I would love to connect."
- End with a low-friction ask (a quick call, 15 minutes, a reply with next steps).
${type === 'connection-request' ? '- HARD LIMIT: Under 300 characters total. Count carefully.' : '- Under 200 words.'}

Output ONLY the message. No subject line. No preamble.`;
}

// ─── Resignation Letter ──────────────────────────────────────────────────────
export function buildResignationPrompt({ type, yourName, managerName, company, role, lastDay, reason, highlights, tone, language }) {
  const lang = language === 'Deutsch'
    ? 'Write entirely in German. Natural, professional German.'
    : 'Write in English.';

  const toneGuide = {
    professional: 'Direct, professional, no excessive warmth. State the facts cleanly.',
    warm: 'Genuinely warm and appreciative — specific about what the role meant. Still professional.',
    brief: 'Very short. State resignation, last day, brief thanks. Under 100 words.',
  };

  const typeContexts = {
    standard: `Standard resignation with appropriate notice period. Positive, professional, no bridges burned.`,
    immediate: `Immediate resignation — leaving without a notice period. Acknowledge this directly and briefly. Do not over-explain or over-apologise. State clearly it is immediate.`,
    retirement: `Retirement announcement. Reflective, warm, grateful. Marks the end of a career chapter.`,
  };

  return `You are writing a resignation letter. Make it dignified, specific, and free of clichés.
${HUMAN_TONE_RULES}

Type: ${typeContexts[type] || typeContexts.standard}
Tone: ${toneGuide[tone] || toneGuide.professional}
${lang}

--- DETAILS ---
Your name: ${yourName || '[Your Name]'}
Manager name: ${managerName || 'not provided — use "Dear [Manager\'s Name]"'}
Company: ${company}
Your role: ${role || 'not specified'}
Last working day: ${lastDay || 'not specified — use "[Date]" as placeholder'}
${reason ? `Reason (mention briefly/naturally if appropriate): ${reason}` : 'Do not mention reason.'}
${highlights ? `Positive highlights to mention: ${highlights}` : ''}
--- END ---

Structure:
1. Clear statement of resignation and last working day (first paragraph)
2. Optional: brief genuine gratitude or highlight — specific, not generic
3. Offer to help transition (standard/warm only — not for immediate)
4. Clean close

Rules:
- Under 200 words (unless tone is warm — max 250).
- No "I have decided to pursue other opportunities" — this is a cliché.
